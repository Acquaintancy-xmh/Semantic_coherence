...
class RobertaForSequenceClassification_v2(BertPreTrainedModel):
    ...
    def __init__(self, config):
        super(RobertaForSequenceClassification_v2, self).__init__(config)
        self.num_labels = 30
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None,
        ...
        h12 = hidden_states[-1][:, 0].reshape((-1, 1, 1024))
        h11 = hidden_states[-2][:, 0].reshape((-1, 1, 1024))
        h10 = hidden_states[-3][:, 0].reshape((-1, 1, 1024))
        h9 = hidden_states[-4][:, 0].reshape((-1, 1, 1024))
...
@add_start_docstrings("""RoBERTa Model transformer with a sequence classification/regression head on top (a linear layer 
    on top of the pooled output) e.g. for GLUE tasks. """,
    ROBERTA_START_DOCSTRING, ROBERTA_INPUTS_DOCSTRING)
class RobertaForSequenceClassification_sepBERT(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels]``.
    ...
    def __init__(self, config):
        super(RobertaForSequenceClassification_sepBERT, self).__init__(config)
        self.num_labels = 30
    ...
    def forward(self, q_input_ids=None, q_attention_mask=None, q_token_type_ids=None,
        ...
        q_outputs = self.roberta(q_input_ids,
        ...
        a_outputs = self.roberta(a_input_ids,
        ...
        cls_embedding = torch.cat((q_outputs[0][:,0,:], a_outputs[0][:,0,:], q_sent_embedding, a_sent_embedding, difference.abs(), point_multi), -1)
